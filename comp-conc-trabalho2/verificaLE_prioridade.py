...
class LE:
	def __init__(self):
		self.escritores = 0
		self.leitores = 0
		self.escritoresQuerendoEscrever=0

	def leitorBloqueado(self,id):
		'''Recebe o id do leitor. Verifica se a decisao de bloqueio esta correta.'''
		if(self.escritores==0 and self.escritoresQuerendoEscrever==0):
			print("ERRO: leitor " + str(id) + " bloqueado quando nao ha escritores e nenhum deles querendo escrever!")

	def escritorBloqueado(self,id):
		'''Recebe o id do escritor. Verifica se a decisao de bloqueio esta correta e registra que ha um escritor querendo escrever.'''
		if(self.escritores==0) and (self.leitores==0):
			print("ERRO: escritor " + str(id) + " bloqueado quando nao ha escritores nem leitores!")
		self.escritoresQuerendoEscrever+=1
	...
	def leitorLendo(self,id):
		...
		if(self.escritoresQuerendoEscrever>0):
			print("ERRO: leitor " + str(id) + " esta lendo quando ha escritor querendo escrever!")
		self.leitores+=1
	...
	def escritorDesbloqueado(self,id):
		'''Recebe o id do escritor e registra o desbloqueio de um escritor que queria executar.'''
		self.escritoresQuerendoEscrever-=1
